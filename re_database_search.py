...
def process_form():
	if request.method == 'POST':
		...
		if type == 'enzyme_name':
			...
			qry = 'SELECT enzyme_name FROM final_project_data WHERE prototype LIKE (%s)'
			curs.execute(qry, (query,))
		...
		elif type == 'source_of_microorganism':
			qry = 'SELECT enzyme_name FROM final_project_data WHERE source_of_microorganism LIKE (%s)'
			curs.execute(qry, (query,))
			for enzyme_name in curs:
				search_results['matches'].append({'enzyme_name': enzyme_name})
				search_results['num_matches'] += 1
		
		elif type == 'recognition_sequence':
			qry = 'SELECT enzyme_name FROM final_project_data WHERE recognition_sequence LIKE (%s)'
			curs.execute(qry, (query,))
			for enzyme_name in curs:
				search_results['matches'].append({'enzyme_name': enzyme_name})
				search_results['num_matches'] += 1
		
		elif type == 'meth_site_type':
			qry = 'SELECT enzyme_name FROM final_project_data WHERE meth_site_type LIKE (%s)'
			curs.execute(qry, (query,))
			for enzyme_name in curs:
				search_results['matches'].append({'enzyme_name': enzyme_name})
				search_results['num_matches'] += 1

		elif type == 'commercial_source':
			qry = 'SELECT enzyme_name FROM final_project_data WHERE commercial_source LIKE (%s)'
			curs.execute(qry, ('%' + query + '%',))
			for enzyme_name in curs:
				search_results['matches'].append({'enzyme_name': enzyme_name})
				search_results['num_matches'] += 1
